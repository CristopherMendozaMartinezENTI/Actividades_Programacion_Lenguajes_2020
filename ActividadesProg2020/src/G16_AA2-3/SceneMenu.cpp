...
#pragma region Title Text
	...
	Renderer::Instance()->LoadRect("titleRect", Rect({ 230, 200, Renderer::Instance()->GetTextureSize(tittleText.id).x, Renderer::Instance()->GetTextureSize(tittleText.id).y }));
	...
	soundButton = { colors["button_c"], colors["button_hover_c"], colors["button_c"], colors["button_hover_c"], "Sound Off", "Sound On",  Rect(SCREEN_WIDTH / 2, 400, 0, 0), "soundButtonText" };
